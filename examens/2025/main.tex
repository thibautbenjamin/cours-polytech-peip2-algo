...
\begin{exercice}[Sommes partielles d'un tableau]
...
\end{lstlisting}
    ...
    \end{enumerate}
  \item On procède ensuite au tri par sélection en miroir, de manière itérative
    \begin{enumerate}
    \item \'Ecrire une fonction
\begin{lstlisting}
...
\end{lstlisting}
      ...
      qui effectue le tri par sélection en miroir sur le tableau. Vous pouvez
  ...
  \item On termine en remarquant qu'il est également possible d'implémenter cet
    ...
    on remarque pour le cas récursif qu'effectuer le tri par sélection en miroir
    ...
    sélection miroir de manière récursive sur les \(n-1\) premiers éléments du
    tableau.
    \begin{enumerate}
    \item \'Ecrivez une fonction
\begin{lstlisting}
void triSelectionMiroir (int n, int tab[])
\end{lstlisting}
      qui implémente la version récursive du tri par sélection. Vous pouvez
      réutiliser la fonction \lstinline|echange| de l'Exercice 1.
...
\end{lstlisting}
    ...
    un entier \lstinline|sommeCourante|, et qui affiche toutes les sommes
...
\end{lstlisting}
    qui prend en argument un entier \(n>0\), un tableau de taille \(\geq n\), et
    qui affiche toutes les sommes possibles obtenues en ajoutant des éléments du
